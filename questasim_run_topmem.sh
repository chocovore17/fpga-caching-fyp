...
if [  -z $1 ] 
then 
vlog -work work +acc=blnr -noincr -timescale 1ns/1ps mem-model-testbench/unit_level/UPDOWNSTREAM/tb_top_UPDOWNSTREAM.sv mem_model_bs/shared/top.sv
...
rm mem-model-testbench/outputs/top/UPDOWNSTREAM_cov_report.txt
vlog -work work +acc=blnr -noincr -timescale 1ns/1ps mem-model-testbench/unit_level/UPDOWNSTREAM/tb_top_UPDOWNSTREAM.sv mem_model_bs/shared/top.sv
vopt -work work tb_top_UPDOWNSTREAM  -o seg_work_opt
vsim -coverage seg_work_opt -c -logfile mem-model-testbench/outputs/top/UPDOWNSTREAM_sim.txt  -do "run -all;  coverage report -file mem-model-testbench/outputs/top/UPDOWNSTREAM_cov_report.txt -byfile -assert -directive -cvg -codeAll; coverage save -onexit -assert -directive -cvg -codeAll mem-model-testbench/outputs/top/updown.ucdb; quit"

# vsim -coverage seg_work_opt -gui -do "run -all; run 1500ns" 

vcover report -details -html mem-model-testbench/outputs/top/updown.ucdb

# firefox covhtmlreport_toplevel/index.html
fi

# Command for top-level testing
# vlog -work work +acc=blnr -noincr -timescale 1ns/1ps  -f mem-model-testbench/ahblite_sys.vc
