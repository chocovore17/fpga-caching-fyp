...
vlog -work work +acc=blnr -noincr -timescale 1ns/1ps testbench/unit_level/UPSTREAM/tb_top_UPSTREAM.sv code/upstream/upstream_top.sv
vopt -work work tb_top_UPSTREAM  -o seg_work_opt
vsim -coverage seg_work_opt -c -logfile testbench/outputs/upstream/UPSTREAMnoreset_sim.txt  -do "run -all; coverage report -file testbench/outputs/upstream/UPSTREAMnoreset_cov_report.txt -byfile -assert -directive -cvg -codeAll; coverage save -onexit -assert -directive -cvg -codeAll testbench/outputs/upstream/upstream.ucdb; quit"
